...
////////////////////////////////////////zbh
#include <svo/map.h>
extern ofstream svo_inf;//("/home/baohua/SLAM/SVO_RGBD_grad/catkin_ws/src/rpg_svo/svo/src/svo_inf.txt");
extern ofstream max_level_tran;
#include<cmath>
//在frame_handler_mono.cpp读取改变金字塔的参数，在此文件和frame.cpp利用
extern float benmark_pyr_ratio;//在frame_handler_mono.cpp定义
extern bool benmark_change_pyr_ratio;//在frame_handler_mono.cpp定义
extern bool Align_level_by_itsOwn_Point;//在frame_handler_mono.cpp定义
extern bool show_message;//在frame_handler_mono.cpp定义
extern bool FastConer0_2_Grad4;//在frame_handler_mono.cpp定义
//////////////////////////////////////////////
namespace svo {
...
/////////////////////////write by zhb   计算前后两帧金字塔每一层的平均灰度值差
    get_avrg_gray_dif_last_and_curframe(); 
/////////////////////////////////////////////////////////
  
  for(level_=max_level_; level_>=min_level_; --level_)
  {
//cout<<avrg_gray_dif_last_and_curframe[level_]<<endl;
    mu_ = 0.1;
...
if(level_==max_level_)
 {
    ...
    Sophus::SE3 max_level_T=T_cur_from_ref;
    Eigen::Vector3d t=max_level_T.inverse().translation();
float R=0,P=0,Y=0;
    //输出RPY角增量
 getRPY(T_cur_from_ref,SE3(Matrix3d::Identity(), Vector3d::Zero()),R,P,Y);
   max_level_tran<<t[0]<<" "<<t[1]<<" "<<t[2]<<" "<<R<<" "<<P<<" "<<Y<<endl;
}
///////////////////////////////
  }
...
float scale;
if(change_pyr_ratio) 
    scale=getscale(level_,pyr_ratio);
else
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
   scale = 1.0f/(1<<level_);
  ...
  {
 ///////////////////////////////zhubaohua////////////////////////////////
              if(Align_level_by_itsOwn_Point==true)        
             {
		if((*it)->level!=level_)
                       continue;
             }
///////////////////////最内层图像对齐只用本层提取的梯度点
              if(FastConer0_2_Grad4)
		{
		  
			if(level_==max_level_)//level4 只用(*it)->level==max_level_的点
                            {
                                   if((*it)->level!=max_level_)
                                   continue;
                               }
	               else if ((*it)->level==max_level_)//其他层不用(*it)->level==max_level_的点
                              continue;
...
jacobian_cache_.col(feature_counter*patch_area_ + pixel_counter) =
            (dx*frame_jac.row(0) + dy*frame_jac.row(1))*(focal_length / getscale(level_,1.0f/pyr_ratio));
else
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
        jacobian_cache_.col(feature_counter*patch_area_ + pixel_counter) =
...
////////////////////////////////////////////////////zhb///////////////////////////////////////////////
        ...
        const float res = intensity_cur - (*ref_patch_cache_ptr)-avrg_gray_dif_last_and_curframe[level_];
///////////////////////////////////////////////////////////////////////////////////////////
   ...
   if(show_message)
{
cout<<"其参考帧上现在的可以用于对齐的特征点个数 ："<<ref_frame_->fts_.size()<<"   本次图像对齐用到的点数： "<<num<<endl;
cout<<"------------------------------本次迭代计算的參差为： "<<chi2/n_meas_<<endl;
}
svo_inf<<"其参考帧上现在的可以用于对齐的特征点个数 ："<<ref_frame_->fts_.size()<<"   本次图像对齐用到的点数： "<<num<<endl;
svo_inf<<"------------------------------本次迭代计算的參差为： "<<chi2/n_meas_<<endl;
  // compute the weights on the first iteration
...
float SparseImgAlign::getscale(int level,float ratio_)
  {
      return pow(ratio_,level);
  }
//////////////////////////write by zhb
void SparseImgAlign::get_avrg_gray_dif_last_and_curframe()
{
  ref_frame_->get_frame_avg_pixel_value();
  cur_frame_->get_frame_avg_pixel_value();
   for(int i=0;i<=max_level_;i++)
    {
   avrg_gray_dif_last_and_curframe.push_back(cur_frame_->avg_pixel_value[i]-ref_frame_->avg_pixel_value[i]);
    }
