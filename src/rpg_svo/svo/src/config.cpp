...
Config::Config() :
    ...
    n_pyr_levels(vk::getParam<int>("svo/n_pyr_levels", 5)),//3
    ...
    grid_size(vk::getParam<int>("svo/grid_size", 30)),//30
    ...
    klt_min_level(vk::getParam<int>("svo/klt_min_level", 1)),
    reproj_thresh(vk::getParam<double>("svo/reproj_thresh", 2.0)),
    poseoptim_thresh(vk::getParam<double>("svo/poseoptim_thresh", 2.0)),//2.0
    ...
    max_fts(vk::getParam<int>("svo/max_fts", 10000)),//120
      #ifdef use_grad
    quality_min_fts(vk::getParam<int>("svo/quality_min_fts", 100)),////30当匹配点小于这个数，跟踪失败
  quality_max_drop_fts(vk::getParam<int>("svo/quality_max_drop_fts", 8000))//40//当后帧匹配到的点的数量和前帧匹配的数量小于这个
//数，重定位。
       #else
         quality_min_fts(vk::getParam<int>("svo/quality_min_fts", 10)),////30当匹配点小于这个数，跟踪失败
    quality_max_drop_fts(vk::getParam<int>("svo/quality_max_drop_fts", 8000))//40//当后帧匹配到的点的数量和前帧匹配的数量小于这个
//数，重定位。
        #endif
#else
