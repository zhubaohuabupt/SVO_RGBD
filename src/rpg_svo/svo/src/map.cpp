...
void getRPY(Sophus::SE3 kf_T,Sophus::SE3 cf_T,float &r,float &p,float &y)
{
	Sophus::SE3 T_cf_kf=cf_T*kf_T.inverse();
	
	Eigen::Matrix3d R(T_cf_kf.rotation_matrix());
	//考虑到slam坐标系，p和y是反着的
...
{
#ifdef yuanlai
              Eigen::Vector3d tmp1=frame->T_f_w_.inverse().translation();////////
          svo_inf<<"cf id :"<<frame->id_<<"位置 "<<tmp1[0]<<" "<<tmp1[1]<<" "<<tmp1[2]<<endl; /////
  for(auto kf : keyframes_)
  {
    ...
    for(auto keypoint : kf->key_pts_)
    {
      ...
      {
          //  Eigen::Vector3d tmp=kf->T_f_w_.translation();////////
            
           // svo_inf<<"overlaps id :"<<kf->id_<<"位置 "<<tmp[0]<<" "<<tmp[1]<<" "<<tmp[2]<<endl; /////

         // Eigen::Vector3d tmp2=(frame->T_f_w_.translation()-kf->T_f_w_.translation());///////
          //  svo_inf<<"位置差异 "<<tmp2[0]<<" "<<tmp2[1]<<" "<<tmp2[2]<<endl; /////
        close_kfs.push_back(
            std::make_pair(
                kf, (frame->T_f_w_.inverse().translation()-kf->T_f_w_.inverse().translation()).norm()));
...
#endif
//把当前帧的上一个关键帧放到close_kfs第一个位置，距离为0;意思是必须要用其进行重投影
close_kfs.push_back(
            std::make_pair(
                frame->last_key_frame, 0.0));       
////////////////////////////////////////////zbh
for(auto kf : keyframes_)
  {
    // check if kf has overlaping field of view with frame, use therefore KeyPoints
    auto keypoint =*(kf->key_pts_.begin());
...
//#define RPY
		...
		getRPY(frame->T_f_w_, kf->T_f_w_.inverse(),R, P, Y);
            
		//float thred=10;
		//if(abs(R)*180/3.14>thred||abs(P)*180/3.14>thred||abs(Y)*180/3.14>thred)  continue;
#endif                  
		////////////////////////////////////////////////////////////////
        close_kfs.push_back(
            std::make_pair(
