...
{
public :
	SGM(int Dmax,int Path):DMax(Dmax),path(Path){
...
int GetDisprity_mat(cv::Mat& img11,cv::Mat &img22,int path,bool debug_display,cv::Mat &LRcheckdisp,cv::Mat &rawdisp);//cv::Mat &rawdisp=nullptr
int Getdisprity_float(float*left,float*right,int path,bool debug_display,int width,int height,float*lrcheckdisp ,float*rawdisp);
